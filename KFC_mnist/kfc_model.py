...
class Model:
    ...
    def compute_fisher(self, imgset, sess, num_samples=200, plot_diffs=False, disp_freq=10,task=None):
        ...
        for i in range(num_samples):
            ...
            # compute first-order derivatives
            # ders = sess.run(tf.gradients(tf.log(probs[0,class_ind]), self.var_list), feed_dict={self.x: imgset[im_ind:im_ind+1]})
            ders = sess.run(self.optim.compute_and_apply_stats(tf.log(probs[0,class_ind]), self.var_list), feed_dict={self.x: imgset[im_ind:im_ind+1]})
            # square the derivatives and add to total
            index = 0
            for keys in ders:
                if i == 0:
                    self.F_accum[-1].append(ders[keys])
                else:
                    self.F_accum[-1][index] += ders[keys]
                    index += 1
            #画图(判断fisher matrix是否收敛)
            if(plot_diffs):
                if i % disp_freq == 0 and i > 0:
    ...
    def update_ewc_loss(self, lam):
        ...
        for i in range(len(self.F_accum)):
            ...
            for v in range(len(self.var_list)):
                if v % 2 == 0:
                    C = int(self.var_list[v].get_shape()[-1])
                    W = tf.transpose(tf.reshape(self.var_list[v] - self.star_vars[v],[-1,C]))
                    right = tf.reshape(tf.transpose(tf.matmul(tf.matmul(self.F_accum[i][j+1] , W),self.F_accum[i][j])),[-1,1])
                    W_ = tf.reshape(self.var_list[v] - self.star_vars[v], [1,-1])
                    self.ewc_loss += tf.matmul(W_, right)
                    j += 2
                else:
                    B  = tf.reshape(self.var_list[v] - self.star_vars[v],[-1,1])
                    right_B = tf.matmul(self.F_accum[i][j], B)
                    B_ = tf.reshape(self.var_list[v] - self.star_vars[v],[1, -1])
                    self.ewc_loss += tf.matmul(B_,right_B)
                    j+=1
