...
def learn(
    ...
    **network_kwargs:   keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, total_timesteps//nbatch+1):
        ...
        if update % 5000 == 0 or update == 1:
            win_rate, lose_rate, tie_rate = test_CNN(model)
            logger.record_tabular("win_rate", win_rate)
            logger.record_tabular("lose_rate", lose_rate)
            logger.record_tabular("tie_rate", tie_rate)
            model.save(save_path + 'updates' + str(update))
            logger.dump_tabular()
