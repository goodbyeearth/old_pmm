...
def learn(
    ...
    **network_kwargs):
    ...
    fisher_matrix2 = joblib.load('fisher_matrix_ewc/simple_agent_3_random_1000')
    fisher_matrix = joblib.load('fisher_matrix_ewc/static_agent_3_random_400')
    # fisher_matrix = joblib.load('fisher_matrix/simple_agent_random_4000')
    # fisher_matrix = joblib.load('fisher_matrix/static_agent_3_random_200')
    # fisher_matrix = joblib.load('fisher_matrix_tf/static_agent_3_random_200')
    fisher_matrix = [fisher_matrix,fisher_matrix2]
    print('fisher matrix done')
    # else:
    #     fisher_matrix = None
    star_param = joblib.load('parameter/parameter79/updates21')
    # star_param = joblib.load('parameter/parameter85/updates2')
    ...
    model = Model(policy=policy, env=env, nsteps=nsteps, ent_coef=ent_coef, vf_coef=vf_coef,
        max_grad_norm=max_grad_norm, lr=lr, alpha=alpha, epsilon=epsilon, total_timesteps=total_timesteps,
                  lrschedule=lrschedule,fisher_matrix=fisher_matrix,star_param=star_param,lam=0,batch_size=batch_size)
    if load_path is not None:
